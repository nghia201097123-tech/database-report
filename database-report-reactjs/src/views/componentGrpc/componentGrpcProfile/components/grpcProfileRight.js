import { Button, Card, Col, Collapse, Row, Select } from "antd";
...
class GrpcProfileRight extends Component {
  static propTypes = {
    project_id: PropTypes.number,
    protoInfo: PropTypes.object.isRequired,
    service_kafka_grpc: PropTypes.array.isRequired,
    service_server_id: PropTypes.number,
    selectedClients: PropTypes.object,
    handleServerChange: PropTypes.func.isRequired,
    handleClientsChange: PropTypes.func.isRequired,
  ...
  componentDidUpdate(prevProps) {
    if (
      prevProps.protoInfo?.service_server_id !==
        this.props.protoInfo?.service_server_id &&
      parseInt(this.props.protoInfo?.service_server_id) > 0 &&
      (!this.props.service_server_id || this.props.service_server_id === 0)
    ) {
      this.props.handleSelectChange(
        parseInt(this.props.protoInfo.service_server_id),
        "service_server_id"
      );
    }
  }
  ...
  render() {
    const { protoInfo } = this.props;

    return (
      protoInfo?.package?.length > 0 && (
        <>
          <Col span={12} style={{ maxHeight: "100vh", overflowY: "auto" }}>
            <Card
              title="Thông tin Proto"
              ...
              extra={
                <div>
                  ...
                  >
                    {parseInt(protoInfo.service_server_id) > 0
                      ? "Cập nhật"
                      : "Tạo mới"}
                  </Button>
                </div>
                    ...
                    <Select
                      ...
                      value={
                        ...
                        (this.props.service_server_id === "" ||
                          ...
                          ? parseInt(protoInfo.service_server_id)
                          : this.props.service_server_id
                      }
                      onChange={(value) =>
                        this.props.handleSelectChange(
            ...
            <Card title="Danh sách Service">
              {Array.isArray(protoInfo.services) &&
                protoInfo.services.map((service, serviceIndex) => (
                  <Card
                    key={serviceIndex}
                    type="inner"
...
export default GrpcProfileRight;
